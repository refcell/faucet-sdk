...
const deployFaucet = async (uri: string, implementationAddress: string, web3: Web3, adapter: string, owner: string, contracts: any, options: any) => {
    // * Deploy FaucetFactory if necessary
    var FaucetFactory = new web3.eth.Contract(JSON.parse(contracts["contracts/FaucetFactory.sol"].abi));;
    ...
    try {
        var receipt = await FaucetFactory.methods.deployFaucet(owner, uri, adapter).send(options);
    } catch (error) {
